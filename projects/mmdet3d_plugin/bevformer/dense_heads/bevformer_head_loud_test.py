...
class BEVFormerHead_LOUD_TEST(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        self.bev_h = bev_h
        self.bev_w = bev_w
        self.topk_u = topk_u # owod parm
        self.topk_o = topk_o # owod parm
        ...
        super(BEVFormerHead_LOUD_TEST, self).__init__(
    ...
    def get_owod_target(self,
        ...
        rpn_select_num = gt_bboxes_list[0].shape[0] + self.topk_o
        ...
        # 筛选 gt，保留 0-4类 进行 GT filtering， 后面的不做操作进行可视化
        valid_indices = [torch.where((labels >= 0) & (labels <= 4))[0] for labels in gt_labels_list]
        
        # 使用有效索引来过滤labels和bboxes
        filtered_gt_labels_list = [labels[idx] for labels, idx in zip(gt_labels_list, valid_indices)]
        filtered_gt_bboxes_list = [bboxes[idx] for bboxes, idx in zip(gt_bboxes_list, valid_indices)]
        
        boxes = proposal_bbox
        gt_boxes = filtered_gt_bboxes_list[0].clone()
        vis_gt_boxes = gt_bboxes_list[0].clone()
        ...
        vis_gt_box_3d_points = get_corners_gt(vis_gt_boxes)
        ...
        vis_gt_x_min, _ = torch.min(vis_gt_box_3d_points[..., 0], dim=1)
        vis_gt_y_min, _ = torch.min(vis_gt_box_3d_points[..., 1], dim=1)
        vis_gt_x_max, _ = torch.max(vis_gt_box_3d_points[..., 0], dim=1)
        vis_gt_y_max, _ = torch.max(vis_gt_box_3d_points[..., 1], dim=1)
        ...
        vis_gt_boxes_img = torch.stack([vis_gt_x_min, vis_gt_y_min, vis_gt_x_max, vis_gt_y_max], dim=1)
      
        # Add the offset to all tensors in one go
        unmatched_boxes += 51.2
        gt_boxes_img += 51.2
        vis_gt_boxes_img += 51.2
        ...
        if  xmin.dim() != 0 and xmin.shape[0] > self.topk_u:
            ...
            _, topk_inds = torch.topk(means_bb_slices, self.topk_u)
            ...
            # unmatched_boxes_print = (unmatched_boxes*10).long().cpu().detach().numpy() # (num_proposals, 4)
            # gt_boxes_print = (gt_boxes_img*10).long().cpu().detach().numpy() # (gt_num, 4)
            # vis_gt_boxes_print = (vis_gt_boxes_img*10).long().cpu().detach().numpy() # (gt_num, 4)
            ...
            # # 创建一个字典，键是标题的一部分，值是相应的框列表
            # boxes_dict = {
            #     'gt_boxes': gt_boxes_print,
            #     'ow_boxes': ow_boxes_print,
            #     'rpn_boxes': rpn_boxes_print,
            #     'vis_gt_boxes': vis_gt_boxes_print,
            #     'unmatched_boxes': unmatched_boxes_print
            # }

            # for i, (title_suffix, boxes_print) in enumerate(boxes_dict.items()):
            #     plt.figure()
            #     fig_path = visual_dir + f'C_BEV_after_transformer_{title_suffix}.png'
            #     ax = sns.heatmap(dense_image_bev_queries.detach().numpy())  # Added ax

            #     # 为每个框绘制矩形
            #     for box in boxes_print:
            #         xmin, ymin, xmax, ymax = box
            #         rect = Rectangle((xmin, ymin), xmax-xmin, ymax-ymin, linewidth=1, edgecolor='b', facecolor='none')
            #         ax.add_patch(rect)
                
            #     plt.gca().invert_yaxis()
            #     plt.title(f'C_BEV_after_transformer_{title_suffix}')
            ...
            original_labels_tensor = filtered_gt_labels_list[0].clone()
            ...
            original_bboxes_tensor = filtered_gt_bboxes_list[0].clone()
            ...
            return filtered_gt_labels_list, filtered_gt_bboxes_list, 0
    ...
    def loss_single_owod(self,
        ...
        # Compute the average number of gt boxes accross all gpus, for
        bbox_pos_num = num_total_pos - self.topk_u
