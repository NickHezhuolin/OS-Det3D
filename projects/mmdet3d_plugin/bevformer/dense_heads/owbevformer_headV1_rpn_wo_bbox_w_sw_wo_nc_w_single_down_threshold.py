...
class OWBEVFormerHeadV1RPNV1_with_soft_weight_without_nc_branch_with_single_down_threshold(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(OWBEVFormerHeadV1RPNV1_with_soft_weight_without_nc_branch_with_single_down_threshold, self).__init__(
    ...
    def get_owod_target(self,
        ...
        # 获取满足阈值条件的索引
        threshold_indices = values < 0.6
        # 根据阈值条件过滤提议框
        filtered_proposal_bbox = all_proposal_bbox[threshold_indices]

        # 如果过滤后的提议框数量少于预期数量，可以选择补充一些置信度最高的提议框
        rpn_select_num = gt_bboxes_list[0].shape[0] + 30
        if filtered_proposal_bbox.shape[0] < rpn_select_num:
            # 对剩余的提议框排序并选择置信度最高的
            sorted_indices = values.argsort(descending=True)
            top_proposal_bbox = all_proposal_bbox[sorted_indices[:rpn_select_num]]
            proposal_bbox = torch.cat((filtered_proposal_bbox, top_proposal_bbox))[:rpn_select_num]
        else:
            # 如果过滤后的提议框数量足够，则直接使用这些框
            proposal_bbox = filtered_proposal_bbox
