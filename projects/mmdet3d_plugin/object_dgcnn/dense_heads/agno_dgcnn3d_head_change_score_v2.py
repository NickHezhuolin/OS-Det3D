...
class AgnoDGCNN3DHeadV3(DETRHead):
    ...
    def __init__(self,
        ...
        super(AgnoDGCNN3DHeadV3, self).__init__(
    ...
    def _get_target_single(self,
        ...
        bboxes_xyz = bbox_pred[:,:3]
        gt_xyz = gt_bboxes[:,:3]
        distances = torch.sqrt(((bboxes_xyz[:, None, :] - gt_xyz[None, :, :]) ** 2).sum(dim=2))
        min_distances = distances.min(dim=1).values
        objectness_targets = torch.exp(-0.1 * min_distances) 
        objectness_weights = gt_bboxes.new_ones(num_bboxes)
        ############################################################################
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list, pos_inds_list, neg_inds_list, 
         objectness_targets_list, objectness_weights_list) = multi_apply( self._get_target_single, cls_scores_list, bbox_preds_list,
    ...
    def loss_single(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
                ...
                objectness_targets_list, objectness_weights_list) = cls_reg_targets
        ...
        loss_obj = self.loss_obj(
            obj_scores[isnotnan], objectness_targets[isnotnan], objectness_weights[isnotnan], avg_factor=objectness_targets.shape[0])
    ...
    def loss(self,
        ...
        for tensor in gt_labels_list:
            ...
            assert not (numpy_array >= 5).any(), "Value greater than or equal to 5 found"
